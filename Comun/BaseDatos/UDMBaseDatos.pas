...
uses
  ...
  UDFactura, AccesoUsuarioFD, Dialogs, DError, Windows, Nb30, WinSock, UDMMaster,
  UDMCalculoPosei;
...
//  DMBaseDatos.dbMoradas.Params.Add('SERVER NAME=server3');
//  DMBaseDatos.dbMoradas.Params.Add('DATABASE NAME=sattfe' );
...
//  DMBaseDatos.dbF17.Params.Add('SERVER NAME=server3');
//  DMBaseDatos.dbF17.Params.Add('DATABASE NAME=chanita' );
  ...
  DMBaseDatos.dbF17.Params.Add('LANGDRIVER=dBASE ESP cp437');
...
//  DMBaseDatos.dbF18.Params.Add('SERVER NAME=server3');
//  DMBaseDatos.dbF18.Params.Add('DATABASE NAME=comerp4h' );
...
begin
  ...
  DMCalculoPosei := TDMCalculoPosei(Application);
end;
...
begin
  ...
  begin
    ...
    bFlag:= true;
    iAux:= -1;
    FDAccesoUsuario:=TFDAccesoUsuario.Create(Application);
    try
      while bFlag do
      begin
        if FDAccesoUsuario.ShowModal = mrOk then
        begin
          gsCodigo := FDAccesoUsuario.teUsuario.Text;
          gsPassword := FDAccesoUsuario.teClave.Text;
          iBDDataConexion:= FDAccesoUsuario.cbbAlias.ItemIndex;
