...
begin
  ...
  begin
    ...
    SQL.Add('      notas_rc notas, fecha_remesa_rc fecha, moneda_cobro_rc moneda, ');
    SQL.Add('      nvl(importe_cobro_rc,0) importe_cobro, ');
    SQL.Add('      sum(importe_cobrado_rf) total_factura ');

    SQL.Add('    from tremesas_cab ');
    SQL.Add('         join tremesas_fac on  empresa_remesa_rc  = empresa_remesa_rf and n_remesa_rc = n_remesa_rf and fecha_remesa_rf = fecha_remesa_rc ');
    ...
    SQL.Add('    and fecha_remesa_rc ' + SQLRangeD(AFechaDesde, AFechaHasta));
    if ACliente <> '' then
