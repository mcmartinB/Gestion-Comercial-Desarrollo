...
uses
  ...
  Grids, DBGrids, ActnList, BusquedaExperta, Rejilla, DPreview,  DError,
  dxSkinsCore, dxSkinBlue, dxSkinBlueprint, dxSkinFoggy, dxSkinMoneyTwins,
  dxSkinscxPCPainter, dxSkinsdxBarPainter, dxSkinBlack, dxSkinCaramel,
...
type
  TFManRemesas = class(TForm)
    ...
    cxLabelSit: TcxLabel;
    deFechaRemesa: TcxDBDateEdit;
    cxLabel9: TcxLabel;
    cxSituacion: TcxLabel;
    ...
    dxBarCambioSit: TdxBarPopupMenu;
    dxPasarAIntroducida: TdxBarButton;
    dxPasarAPdteContabilizar: TdxBarButton;
...
//    procedure dxAceptarClick(Sender: TObject);
    ...
    procedure dxCambioSitClick(Sender: TObject);
    ...
    procedure dxPasarAIntroducidaClick(Sender: TObject);
    procedure dxPasarAPdteContabilizarClick(Sender: TObject);
    procedure ssEmpresaClick(Sender: TObject);
    procedure ssClienteClick(Sender: TObject);
    procedure ssMonedaClick(Sender: TObject);

  private
    bAlta, bCancelar, bAltaLinea, bModLinea, bPrimeraLin,
    bErrorLin, bModCabecera, bModificacion: Boolean;
    QRemesaCab, QRemesaFac : TBonnyClientDataSet;
//    QRemesaCab, QRemesaFac : TBonnyQuery;
    QImpRemesa: TBonnyQuery;
    iFacturas, iNumRemesa, iSituacion: integer;
    cImpFacturas, cImporteOld, rImpPendiente: currency;
    QDatos, QDatosImp, QContRemesa, QDelFacturas, QFactura, QGrupo,
    QEmpresaRem, QDesCliente, QImpCobrado, QImpCobrado2, QDatosCliente, QFacturas, QRemesaFacSum : TBonnyQuery;
    dFechaRemesa, dFechaVtoRem, dFechaDesRem: TDateTime;
...
//    procedure MostrarConsulta;
    ...
    procedure PasarAIntroducida;
    procedure PasarAFinalizada;
    procedure PasarAPdteContabilizar;
    procedure ContabilizarRem;
    function MostrarCliente(ACodigoFactura: String): String;
    procedure CalcImpFacturas( var rImpFacturas: real; var iCanFacturas:integer );

    procedure PonSituacion;
    procedure MostrarUltimaActualizacion;
    procedure ActualizaSituacion( const ASituacion: integer );
    function ExisteFacturaNoComercial: Boolean;
...
//  QRemesaCab:=TBonnyQuery.Create(self);
...
//    RequestLive := true;
    ...
    SQL.Add(' order by empresa_remesa_rc, fecha_remesa_rc ');
  end;
end;
...
begin
//  QRemesaFac:=TBonnyQuery.Create(self);
 ...
 //   RequestLive := true;
    ...
    SQL.Add('   and fecha_remesa_rf = :fecha ');
    ParamByName('empresa').DataType := ftString;
...
procedure TFManRemesas.CreaQRemesaFacSum;
begin
  QRemesaFacSum:=TBonnyQuery.Create(self);
  with QRemesaFacSum do
  begin
    SQL.Add(' select sum(importe_cobrado_rf) totalfac, count(*) canfac');
    SQL.Add(' from tremesas_fac ');
    SQL.Add(' where empresa_remesa_rf = :empresa ');
    SQL.Add('   and n_remesa_rf = :remesa ');
    SQL.Add('   and fecha_remesa_rf = :fecha ');
...
begin
  ...
  begin
    ...
    ParamByName('fecha').AsString := QRemesaCab.FieldByName('fecha_remesa_rc').AsString;
...
procedure TFManRemesas.CalcImpFacturas( var rImpFacturas: real; var iCanFacturas: integer );
begin
  with QRemesaFacSum do
  begin
    if Active then
    ...
    Open;
    rImpFacturas := FieldByName('TotalFac').AsFloat;
    iCanFacturas := FieldByName('CanFac').AsInteger;
  end;
end;
...
begin
  ...
  begin
    ...
    SQL.Add('    where cliente_c = cod_cliente_rc ');
{
    ...
    SQL.Add('    and fecha_remesa_rc = :fecharemesa ');
...
begin
  ...
  begin
    ...
    ParamByName('fecharemesa').AsString := QRemesaCab.FieldByName('fecha_remesa_rc').AsString;
...
var sLista: String;
begin
  TryStrToDate( deFechaRemesa.Text, dFechaRemesa );
...
begin
  dxBaja.Enabled := AActivar and (QRemesaCab.FieldByName('situacion_rc').AsInteger = 10) and gbEscritura;
  dxModificar.Enabled := AActivar and (QRemesaCab.FieldByName('situacion_rc').AsInteger = 10 ) and gbEscritura;
  dxManFactura.Enabled := AActivar;
  dxImprimir.Enabled := AActivar;
  dxSalir.Enabled := AActivar;
  gbDetalle.Enabled := true;    //siempre activo para poder utilizar el scroll
  pnlAltaLineas.Enabled := (bAltaLinea) or(bModLinea) or (AActivar and (QRemesaCab.FieldByName('situacion_rc').AsInteger = 10));
  cxTabControl1.Enabled := (bAltaLinea) or(bModLinea) or (AActivar and (QRemesaCab.FieldByName('situacion_rc').AsInteger = 10));
  //Situacion Remesa
  case iSituacion of
    10:   //Situacion Introducida
    begin
      dxCambioSit.ButtonStyle := bsDefault;
      dxCambioSit.Caption := 'Pasar a Finalizada';
      dxCambioSit.Enabled := AActivar and gbEscritura;
    end;
    20:   //Situacion Finalizada
    begin
      dxCambioSit.ButtonStyle := bsDropDown;
      if gbEnlaceContable then
      begin
        dxCambioSit.Caption := 'Pasar a Pdte. Contabilizar';
        dxCambioSit.Enabled := AActivar and gbEnlaceContable;
      end
      else
      begin
        dxCambioSit.Caption := 'Pasar a Introducida';
        dxCambioSit.Enabled := AActivar and gbEscritura;
      end;

      dxPasarAIntroducida.Enabled := AActivar and not bAlta and gbEscritura;
      dxPasarAPdteContabilizar.Enabled := AActivar and gbEnlaceContable;
    end;
    30:  //Situacion Contabilizada
    begin
      dxCambioSit.ButtonStyle := bsDefault;
      dxCambioSit.Caption := 'Pasar a Finalizada';
      dxCambioSit.Enabled := AActivar and gbEnlaceContable;
    end;
    40:   //Situacion Pdte. Contabilizar
    begin
      dxCambioSit.ButtonStyle := bsDefault;
      dxCambioSit.Caption := 'Pasar a Pdte. Contabilizar';
      dxCambioSit.Enabled := AActivar and gbEnlaceContable;
    ...
    begin
      dxCambioSit.ButtonStyle := bsDefault;
      dxCambioSit.Caption := 'Situacion Remesa';
      dxCambioSit.Enabled := AActivar and gbEnlaceContable;
    end;
  end;
end;
...
begin
  ...
  QRemesaCab.FieldByName('fecha_remesa_rc').AsDateTime := Date;
  ...
  QRemesaCab.FieldByName('situacion_rc').AsInteger := 10;
  QRemesaCab.FieldByName('usuario_situacion_rc').AsString := gsCodigo;
...
begin
  ...
  QRemesaFac.FieldByName('fecha_remesa_rf').AsString := QRemesaCab.FieldByName('fecha_remesa_rc').AsString;
...
begin
  ...
  begin
    ...
    //Fecha Remesa
  if not TryStrToDate( deFechaRemesa.Text, dFechaRemesa ) then
  begin
    ShowMessage( 'La fecha de Remesa es incorrecta.');
    deFechaRemesa.SetFocus;
    exit;
  end;
    //Fecha Vencimiento
  if not TryStrToDate( deFechaVtoRem.Text, dFechaVtoRem ) then
  begin
    ShowMessage( 'La fecha de Vencimiento es incorrecta.');
  ...
  *)
  ...
  if not ChangeExist( edtMonedaRem.Text, deFechaRemesa.Text ) then
  begin
    ceBrutoRem.Text:= '0';
    ShowMessage('Falta grabar el cambio de "' + edtMonedaRem.Text + '" ' +
                           'del dia "' + deFechaRemesa.Text + '".');
    deFechaRemesa.SetFocus;
    exit;
  end;
...
procedure TFManRemesas.PonSituacion;
begin
  iSituacion := QRemesaCab.Fieldbyname('situacion_rc').AsInteger;
  case iSituacion of
    10: cxSituacion.Caption := '10 - Introducida';
    20: cxSituacion.Caption := '20 - Finalizada';
    30: cxSituacion.Caption := '30 - Pdte. Contabilizar';
    40: cxSituacion.Caption := '40 - Contabilizada';
    else cxSituacion.Caption := 'Situacion Remesa'
  end;
end;
...
begin
  ...
  begin
    ...
    SQL.Add(StringSQL);
    SQL.Add(' order by empresa_remesa_rc, fecha_remesa_rc, n_remesa_rc ');
  end;
...
begin
  ...
  cxDiferencia.Style.Font.Color := clBtnFace;
  iSituacion := 0;
  cxSituacion.Caption := 'Situacion Remesa';
...
begin
  ...
  begin
    ...
    SQL.Clear;
    SQL.Add(' select cod_factura_rf from tremesas_fac    ');
    SQL.Add('  where empresa_remesa_rf = :empresa ');
    SQL.Add('    and fecha_remesa_rf = :fecha ');
    SQL.Add('    and n_remesa_rf = :numero ');

    ParamByName('empresa').AsString := QRemesaCab.Fieldbyname('empresa_remesa_rc').AsString;
    ParamByName('fecha').AsString := QRemesaCab.Fieldbyname('fecha_remesa_rc').AsString;
    ParamByName('numero').AsInteger:= QRemesaCab.Fieldbyname('n_remesa_rc').AsInteger;
    Open;

    while not DFactura.QAux.Eof do
    begin
      if DFactura.QAux.FieldByName('cod_factura_rf').AsString <> '' then
      begin
        if bPrimera then
          bPrimera := False
        else
        ...
        sFacturas := sFacturas + QuotedStr(DFactura.QAux.FieldbyName('cod_factura_rf').AsString);
      end;
...
function TFManRemesas.ExisteFacturaNoComercial: Boolean;
begin
  with DFactura.QAux do
  begin
    ...
    SQL.Add('    and fecha_remesa_rf = :fecha ');
    SQL.Add('    and n_remesa_rf = :numero ');
    SQL.Add('    and es_fac_comercial_rf = 0 ');
...
procedure TFManRemesas.MostrarUltimaActualizacion;
begin
  BETexto('', 'Ultima actualización: ' + QRemesaCab.fieldbyname('fecha_situacion_rc').AsString + '     Usuario: ' +
                                         DesUsuario(QRemesaCab.fieldbyname('usuario_situacion_rc').AsString));
end;

procedure TFManRemesas.ActualizaSituacion(const ASituacion: integer);
begin

    if not (QRemesaCab.State in dsEditModes) then
      QRemesaCab.Edit;

    QRemesaCab.FieldByName('situacion_rc').AsInteger := ASituacion;
    QRemesaCab.FieldByName('fecha_situacion_rc').AsDateTime := Now;
    QRemesaCab.FieldByName('usuario_situacion_rc').AsString := gsCodigo;
    QRemesaCab.Post;
...
begin
  ...
  begin
    Showmessage(' ATENCION! Esta remesa no tiene lineas de facturas. ');
    exit;
  end;
  try
       //Crear informe
...
begin
  case key of
    ...
    begin
      ...
      else if (gbDetalle.Enabled) and (btnAltaLinea.Enabled) and (QRemesaCab.FieldByname('situacion_rc').AsInteger = 10) then btnAltaLineaClick(Self);
//      else if (gbDetalle.Enabled) and (btnAltaLinea.Enabled) and (not cbRemContabilizada.Checked) then btnAltaLineaClick(Self);
    ...
    begin
      ...
      else if not (bAlta or bModCabecera) and (gbDetalle.Enabled) and (btnBajaLinea.Enabled) and (QRemesaCab.FieldByname('situacion_rc').AsInteger = 10) then btnBajaLineaClick(Self);
    end;
    vk_ESCAPE:
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TryStrToDate( QRemesaCab.FieldByName('fecha_remesa_rc').AsString, dAux ) then
          iAux:= ChangeToEuro( QRemesaCab.FieldByName('moneda_cobro_rc').AsString,
                               QRemesaCab.FieldByName('fecha_remesa_rc').AsString, iAux, 2)
        else
          iAux:= 0;
      end
      else
...
//  dxImprimir.Enabled := false;
  BtnNavegador;
  ActualizaSituacion( QRemesaCab.Fieldbyname('situacion_rc').AsInteger );
end;
...
begin
  if QRemesaFac.RecordCount = 0 then  exit;
...
begin
  ...
  BERemesas.setValor('fecha_remesa_rc', '>=' + DateToStr(EncodeDate(Ano, 1, 1)));
  BERemesas.SetValor('situacion_rc','{10,20,30,40}');
...
procedure TFManRemesas.dxCambioSitClick(Sender: TObject);
//var sAux: String;
begin
  case QRemesaCab.FieldByName('situacion_rc').AsInteger of
    10:   //Situacion Introducida
    begin
      PasarAFinalizada;
    end;
    ...
    begin
      if gbEnlaceContable then
        PasarAPdteContabilizar
      else
        PasarAIntroducida;
    end;
    30:  //Situacion Pdte Contabilizar
    begin
      PasarAFinalizada;
    end;
    40:  //Situacion Contabilizada
    begin
      PasarAPdteContabilizar;
    end;
  end;
...
//    exit;
  ...
  begin
    ...
    dxCambioSit.Enabled := true and gbEnlaceContable;
...
procedure TFManRemesas.PasarAIntroducida;
var sAux: String;
begin

  sAux := 'Se va a pasar la Remesa a INTRODUCIDA. ¿Continuar con el proceso?';

  case MessageDlg(sAux, mtInformation, [mbCancel,mbOk],0) of
  mrOk:
  begin
    ActualizaSituacion( 10 );
//    QRemesaCab.ApplyUpdates(0);
//    ShowMessage(' Remesa INTRODUCIDA');
    BtnNavegador;
    ActivarEdicion(true);
  end;
  mrCancel:
...
procedure TFManRemesas.PasarAFinalizada;
var sAux: String;
begin

  if QRemesaFac.RecordCount = 0 then
  begin
    ShowMessage('ATENCION! No se puede Finalizar la Remesa. ' + #13 + #10 +    #13 + #10 +
                'La Remesa no tiene lineas de Facturas.');
    exit;
  end;
  if not ImporteCorrecto then
  begin
  ...
  if ExisteFacturaNoComercial then
  begin
    ShowMessage('ATENCION! No se puede Finalizar la Remesa. ' + #13 + #10 + #13 + #10 +
                'Alguna de las facturas / abonos, no existen en el sistema.');
    exit;
  end;

  sAux := 'Se va a pasar la Remesa a FINALIZADA. ¿Continuar con el proceso?';

  case MessageDlg(sAux, mtInformation, [mbCancel,mbOk],0) of
  mrOk:
  begin
      ActualizaSituacion( 20 );
//    QRemesaCab.ApplyUpdates(0);
//    ShowMessage(' Remesa FINALIZADA');
    BtnNavegador;
    ActivarEdicion(true);
  end;
  mrCancel:
...
procedure TFManRemesas.PasarAPdteContabilizar;
var sAux: String;
begin

  sAux := 'Se va a pasar la Remesa a PDTE. CONTABILIZAR. ¿Continuar con el proceso?';

  case MessageDlg(sAux, mtInformation, [mbCancel,mbOk],0) of
  mrOk:
  begin
    ActualizaSituacion( 30 );
//    QRemesaCab.ApplyUpdates(0);
//    ShowMessage(' Remesa PDTE. CONTABILIZAR');
    BtnNavegador;
    ActivarEdicion(true);
  end;
  mrCancel:
...
begin
  if (QRemesaCab.FieldByName('situacion_rc').AsInteger > 10) or
...
var sMSg: string;
begin
  if QRemesaCab.FieldByName('situacion_rc').AsInteger <> 10 then
  begin
    ShowMessage('No se puede borrar una remesa que ya ha sido finalizada o contabilizada.');
    Exit;
  end;
...
begin
  ...
  if sFacturas = '' then
  begin
    ShowMessage('ATENCION! Remesa sin lineas de facturas.');
    Exit;
  end;
  BETexto('','');
  ...
  try
    ...
    FManFacturas.FormStyle := fsMDIForm;
    FManFacturas.Visible := false;

    FManFacturas.MostrarFacturasExt(sFacturas);
    FManFacturas.ShowModal;
  finally
    FreeAndNil(FManFacturas );
  end;
...
procedure TFManRemesas.ssEmpresaClick(Sender: TObject);
begin
  if QRemesaFac.RecordCount <> 0 then exit;
end;

procedure TFManRemesas.ssMonedaClick(Sender: TObject);
begin
  if QRemesaFac.RecordCount <> 0 then exit;
end;
...
procedure TFManRemesas.ssClienteClick(Sender: TObject);
begin
  if QRemesaFac.RecordCount <> 0 then exit;
end;
...
begin
  if QRemesaCab.FieldByName('situacion_rc').AsInteger <> 10 then
  begin
    ShowMessage('No se puede borrar una remesa que ya ha sido finalizada / contabilizada.');
    Exit;
  end;
...
procedure TFManRemesas.dxPasarAIntroducidaClick(Sender: TObject);
begin
  PasarAIntroducida;
end;

procedure TFManRemesas.dxPasarAPdteContabilizarClick(Sender: TObject);
begin
  PasarAPdteContabilizar;
end;
...
procedure TFManRemesas.dsQRemesaCabDataChange(Sender: TObject;
  Field: TField);
  var rImpFacturas: real;
      iCanFacturas: integer;
begin
  BtnNavegador;
  PonSituacion;
  if (not bModCabecera) and (not bAlta) then
  begin
    ActivarEdicion(True);
  end;
  ...
  begin
    ...
    CalcImpFacturas( rImpFacturas, iCanFacturas );
    cImpFacturas := rImpFacturas;
    txNumFacturas.Text := IntToStr(iCanFacturas);
    txImpFacturas.Text := FormatFloat( '#0.00', rImpFacturas );
    CalcDiferencia;
    MostrarUltimaActualizacion;
  end;
end;
